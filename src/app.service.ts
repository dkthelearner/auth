...
import { compareHash, ConfigService, createHash, PrismaService, TokenService } from './core/services';
import { Role, Token, TokenStatus, User } from '@prisma/client';
...
export class AppService {
    ...
    public async changePassword(data: ForgotPasswordDto, authUserId: number): Promise<void> {
        try {
            ...
            if (!getActiveToken) {
                throw new HttpException('active_token_not_found', HttpStatus.BAD_REQUEST);
            }
            const addExp = moment().add(Number(this.configService.get('tokenExp')), 'second');
            if (moment(getActiveToken.created_at).isAfter(addExp)) {
                throw new HttpException('forgot_token_expired', HttpStatus.INTERNAL_SERVER_ERROR);
            }
